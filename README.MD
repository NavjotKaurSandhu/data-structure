############# See data-structure folder ###########

# What is Data Structure :
1. A way of organizing data that is stored in a computer or database
2. Many different types of data stuctures
3. Each type of data structure represents a different way of organizing the data.
...
1. Can have big impact on how performant, quick and efficiently a program runs.
    -> In case if you want to store data in database but don't want to retrieve in that case Linked List is the example which is really fast at recording data even though it's not fastest at retrieving that data.
    -> In case if you want to access data, we can use Hash Table.
2. Reinforce knowledge of JS algo and other important concepts.
...
# Prototype : It simply an object that multiple other objects that can refer to get any information or functionlaity that they need.
...
Ways to create and initialize arrays :
...
JavaScript only supports one-dimensional arrays; it does not support matrices. However, we can implement matrices or any multi-dimensional array using an array of arrays:
    let averageTemp = []; 
...
* Iterating an array
    -> for...of : (ES2015 or ES6) Iterates through the values of an array.
    -> every : iterates each element of the array untill function returns false. If condition returns false then array iteration will not happen for rest of elements in an array.
    -> some : iterates each elment of the array untill the return of function is true. If condition returns true then array iteration will not happen for rest of elements in an array.
    -> forEach : If we need the array to be completely iterated no matter what, we can use the forEach function.
    -> map : It returns new array with a result. For each element is returns the result where it is true or false. It convert each item of an array.
    -> filter : It returns new array with a result. It returns new array which include only true condition. It is used to select certain items of an array.
    -> reduce : It is used to reduce the array to one single value. It reduces the array into one single value and returns it upon completion.
    -> @@iterator : (ES2015 or ES6) It is used to loop over any collection in JavaScript. 
        * If we have object inside object and that object contain arrays, to loop this type of object we use it. 
    ...
    -> entries : (ES2015 or ES6) It returns a new Array Iterator object that contains the key/value pairs for each index in the array.
        * To be able to retrieve key/value pairs is very useful when we are working with sets, dictionaries, and hash maps.
    -> keys : (ES2015 or ES6) It returns @@iterator, which contains the keys of the array.
    -> values : It returns @@iterator, which contains the values of the array.
    -> from : (ES2015 or ES6) The Array.from() method creates a new, shallow-copied Array instance from an array-like or iterable object.
        ...
        * It is to convret kinds of objects to arrays :
            * Array-like objects : which has property length and indexed elements. Eg: DOM operation such as document.getElementByClassName().
    ...
    -> of : (ES2015 or ES6) : Array.of provides a constructor that, unlike Array, does not have the special case for new Array(42), which presets length.
    ...
    -> copyWithin : (ES2015 or ES6) : It copies a sequence of values of the array into the position of a start index.
...
# TypedArray : It is used for handling binary data.
...
# Field or Field Proposal : We will be able to declare private properties by prefixing the properties with the hash symbol (#).
